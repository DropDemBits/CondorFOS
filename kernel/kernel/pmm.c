...
{
    if(addr == 0 || addr == (physical_addr_t) stack_base) return;
    
    if(stack_offset >= ADDR_PER_BLOCK-1)
    {
        physical_addr_t prev_stack = (physical_addr_t)stack_base;
        stack_base = (physical_addr_t*) addr;
...
void pmm_setRegion(physical_addr_t region_start, size_t region_size)
{
    ...
    regions_base[region_indice].flags = 0x3;
    region_indice++;
}
...
void pmm_clearRegion(physical_addr_t region_start, size_t region_size)
{
    ...
    regions_base[region_indice].flags = 0x1;
    region_indice++;
}
